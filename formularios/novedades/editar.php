...
$sql = "SELECT * FROM acta_novedad WHERE id = ?";
...
$datos = [
    ...
    'detalle_novedad' => $registro['detalle_novedad'],
    'descargos_ppl' => $registro['descargos_ppl']
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        ...
        $sql = "UPDATE acta_novedad 
                SET dni_ppl = ?, fecha = ?, hora = ?, id_sector = ?, id_pabellon = ?, 
                    detalle_novedad = ?, descargos_ppl = ?, id_usuario = ?
                WHERE id = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                $_SESSION['mensaje_exito'] = "✅ Acta de novedad actualizada correctamente";
                header("Location: index.php");
                exit();
            } else {
...
<head>
    ...
    <title>Editar Acta de Novedad</title>
    <style>
            ...
            <div class="acta-content">
                <div class="acta-paragraph">
                    ...
                    <input type="time" id="hora" name="hora" class="inline-field field-time" 
                           value="<?php echo htmlspecialchars($datos['hora']); ?>" required> hs, a los
