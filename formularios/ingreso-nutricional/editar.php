...
$sql = "SELECT * FROM ingreso_nutricional WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $datos['certificacion_med'] = trim($_POST['certificacion_med'] ?? '');
    ...
    if (empty($errores)) {
        ...
        $sql = "UPDATE ingreso_nutricional 
                SET dni_ppl = ?, fecha_ingreso = ?, peso_kg = ?, talla_m = ?, diagnostico = ?, id_dieta = ?, 
        ...
        if ($stmt) {
            $stmt->bind_param(
                ...
                $datos['diagnostico'],
                $datos['id_dieta'],
                $datos['antecedentes_pat'],
                $certificacion_med_bool,
                $datos['firma_ppl'],
            ...
            if ($stmt->execute()) {
                $_SESSION['mensaje_exito'] = "✅ Planilla de ingreso nutricional actualizada correctamente";
                header("Location: index.php");
                exit();
            } else {
...
<head>
    ...
    <title>Editar Ingreso Nutricional</title>
    <style>
        body {
    ...
    <div class="page-a4">
        ...
        <form method="POST" action="">
            ...
            <div class="actions">
                ...
                <button type="submit" class="btn">Actualizar Planilla</button>
            </div>
        </form>
    </div>
