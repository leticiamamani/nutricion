...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        ...
        if ($datos['tipo_producto'] === 'LECHE_DESLACTOSADA') {
            // Para leche: usar cantidad y fecha_vto
            ...
            // Para galletas+pan: usar las nuevas columnas
            $cantidad = NULL;
            $fecha_vto = NULL;
        ...
        $sql = "INSERT INTO entrega_productos 
                (fecha, id_sector, id_pabellon, dni_ppl, tipo_producto, cantidad, fecha_vto, 
                 cantidad_galletas, fecha_vto_galletas, cantidad_pan, fecha_vto_pan,
                 firma_ppl, aclaracion, firma_efectivo, id_usuario) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                $_SESSION['mensaje'] = "✅ Entrega de producto registrada correctamente";
...
<head>
    ...
    <title>Agregar Acta de Entrega</title>
    <style>
        body {
