...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    $_SESSION['mensaje_error'] = "❌ No se especificó la distribución a editar";
...
// Obtener datos de la distribución actual
$query_distribucion = "SELECT da.*, 
                              s.nombre as sector_nombre,
                              da.id_sector_distribucion
...
$stmt_distribucion->bind_param("i", $id_distribucion);
...
$distribucion = $result_distribucion->fetch_assoc();
...
// Obtener todos los registros para esta fecha y tipo de comida
$fecha = $distribucion['fecha'];
$tipo_comida_id = $distribucion['id_tipo_comida'];

$query_todos = "SELECT da.*, 
                       s.nombre as sector_nombre
                FROM distribucion_alimentos da 
                LEFT JOIN sectores_distribucion s ON da.id_sector_distribucion = s.id
                WHERE da.fecha = ? AND da.id_tipo_comida = ?";
$stmt_todos = $conexion->prepare($query_todos);
$stmt_todos->bind_param("si", $fecha, $tipo_comida_id);
$stmt_todos->execute();
$result_todos = $stmt_todos->get_result();

// Inicializar datos para cada sector
$datos_sectores = [];
foreach ($sectores as $sector_id => $sector_nombre) {
    $datos_sectores[$sector_id] = [
        'id' => '',
        'nro_colaciones' => '',
        'pan_kg' => '',
        'te_mate_cocido' => '',
...
while ($row = $result_todos->fetch_assoc()) {
    $sector_id = $row['id_sector_distribucion'];
    if (isset($datos_sectores[$sector_id])) {
        $datos_sectores[$sector_id] = [
            'id' => $row['id'],
            'nro_colaciones' => $row['nro_colaciones'] ?? '',
            'pan_kg' => $row['pan_kg'] ?? '',
            'te_mate_cocido' => $row['te_mate_cocido'] ?? '',
            'nro_dietas' => $row['nro_dietas'] ?? '',
            'nro_viandas_comunes' => $row['nro_viandas_comunes'] ?? '',
            'hora_llegada' => $row['hora_llegada'] ?? '',
            'hora_recibido' => $row['hora_recibido'] ?? '',
            'firma_sector' => $row['firma'] ?? '',
            'aclaracion_sector' => $row['aclaracion'] ?? ''
        ];
    }
}
...
$errores = [];
$datos = [
    'id_tipo_comida' => $distribucion['id_tipo_comida'],
    'fecha' => $distribucion['fecha'],
    'hora' => $distribucion['hora']
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $datos['id_tipo_comida'] = intval($_POST['id_tipo_comida'] ?? 1);
    $datos['fecha'] = trim($_POST['fecha'] ?? '');
    $datos['hora'] = trim($_POST['hora'] ?? '');

    // Recoger datos por sector (por ID)
    foreach ($sectores as $sector_id => $sector_nombre) {
        $datos_sectores[$sector_id]['nro_colaciones'] = $_POST['nro_colaciones'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['pan_kg'] = $_POST['pan_kg'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['te_mate_cocido'] = $_POST['te_mate_cocido'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['nro_dietas'] = $_POST['nro_dietas'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['nro_viandas_comunes'] = $_POST['nro_viandas_comunes'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['hora_llegada'] = $_POST['hora_llegada'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['hora_recibido'] = $_POST['hora_recibido'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['firma_sector'] = $_POST['firma_sector'][$sector_id] ?? '';
        $datos_sectores[$sector_id]['aclaracion_sector'] = $_POST['aclaracion_sector'][$sector_id] ?? '';
    }
    ...
    if (empty($errores)) {
        $success_count = 0;
        $tipo_comida_nombre = $tipos_comida[$datos['id_tipo_comida']] ?? 'DESAYUNO';
        
        // Actualizar datos para cada sector
        foreach ($sectores as $sector_id => $sector_nombre) {
            $data = $datos_sectores[$sector_id];
            
            // Determinar la descripción según el tipo de comida
            $descripcion = '';
            switch($datos['id_tipo_comida']) {
                case 1: // DESAYUNO
            ...
            if (!empty($data['id'])) {
                ...
                $sql = "UPDATE distribucion_alimentos 
                        SET id_tipo_comida = ?, fecha = ?, hora = ?, id_sector_distribucion = ?, nro_colaciones = ?, 
                            nro_dietas = ?, nro_viandas_comunes = ?, pan_kg = ?, te_mate_cocido = ?, 
                            hora_llegada = ?, hora_recibido = ?, firma = ?, aclaracion = ?
                        WHERE id = ?";
                
                $stmt = $conexion->prepare($sql);
                
                if ($stmt) {
                    // CORREGIDO: Cadena de tipos con 14 caracteres para 14 parámetros
                    $stmt->bind_param(
                        "issiiiissssssi", // 14 caracteres - CORREGIDO
                        $datos['id_tipo_comida'],
                        $datos['fecha'],
                        $datos['hora'],
                        $sector_id,
                        $nro_colaciones,
                    ...
                    if ($stmt->execute()) {
                        ...
                        $errores[] = "Error al actualizar sector $sector_nombre: " . $stmt->error;
                    }
                    $stmt->close();
                } else {
                    $errores[] = "Error preparando consulta para sector $sector_nombre: " . $conexion->error;
                }
            } else {
                // Insertar nuevo registro si no existe pero tiene datos
                if (tieneDatos($data)) {
                    $sql = "INSERT INTO distribucion_alimentos 
                            (id_tipo_comida, fecha, hora, id_sector_distribucion, nro_colaciones, 
                             nro_dietas, nro_viandas_comunes, pan_kg, te_mate_cocido, 
                             hora_llegada, hora_recibido, firma, aclaracion, id_usuario) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    ...
                    if ($stmt) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            $errores[] = "Error al insertar sector $sector_nombre: " . $stmt->error;
                        }
                        $stmt->close();
                    } else {
                        $errores[] = "Error preparando consulta de inserción para sector $sector_nombre: " . $conexion->error;
                    }
                }
            }
        }
        ...
        if ($success_count > 0) {
            $_SESSION['mensaje_exito'] = "✅ Distribución de " . $tipo_comida_nombre . " actualizada correctamente para $success_count sectores";
            header("Location: index.php");
            exit();
        } else {
            $errores[] = "No se actualizó ningún registro. Verifique que haya ingresado datos en al menos un sector.";
        }
    }
}
?>
...
<head>
    ...
    <title>Editar Distribución de Alimentos - Área Nutrición</title>
    <style>
        body {
...
<body>
    ...
    <div class="page-a4">
        <div class="header">
            ...
            <h3>EDITAR DISTRIBUCIÓN DE ALIMENTOS</h3>
        </div>
        ...
        <div class="info-box">
            <strong>💡 EDITANDO:</strong> Está editando la distribución de <?php echo $tipos_comida[$datos['id_tipo_comida']]; ?> del <?php echo $datos['fecha']; ?>
        </div>

        <div class="comida-section">
            <form method="POST" action="" class="comida-form" id="formulario-unico">
                <input type="hidden" name="id_tipo_comida" id="id_tipo_comida" value="<?php echo $datos['id_tipo_comida']; ?>">
                ...
                <div class="section-header">
                    <div class="comida-selector">
                        ...
                        <select id="selector-comida" name="selector_comida">
                            <?php foreach ($tipos_comida as $tipo_id => $tipo_nombre): 
                                $selected = ($tipo_id == $datos['id_tipo_comida']) ? 'selected' : '';
                                $icono = '';
                                switch($tipo_nombre) {
                    ...
                    <div class="fecha-hora">
                        📅 Fecha: 
                        <input type="date" name="fecha" value="<?php echo htmlspecialchars($datos['fecha']); ?>" class="inline-field field-small" required>
                        🕒 Hora: 
                        <input type="time" name="hora" value="<?php echo htmlspecialchars($datos['hora']); ?>" class="inline-field field-small">
                    </div>
                </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="number" name="nro_colaciones[<?php echo $sector_id; ?>]" 
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['nro_colaciones']); ?>" 
                                ...
                                <td class="desayuno-fields">
                                    <input type="number" name="pan_kg[<?php echo $sector_id; ?>]" step="0.01" min="0"
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['pan_kg']); ?>" 
                                           class="number-input" placeholder="0.00">
                                </td>
                                <td class="desayuno-fields">
                                    <select name="te_mate_cocido[<?php echo $sector_id; ?>]">
                                        <option value="">-</option>
                                        <option value="TÉ" <?php echo ($datos_sectores[$sector_id]['te_mate_cocido'] === 'TÉ') ? 'selected' : ''; ?>>TÉ</option>
                                        <option value="MATE_COCIDO" <?php echo ($datos_sectores[$sector_id]['te_mate_cocido'] === 'MATE_COCIDO') ? 'selected' : ''; ?>>MATE COCIDO</option>
                                    </select>
                                </td>
                                ...
                                <td class="almuerzo-fields">
                                    <input type="number" name="nro_dietas[<?php echo $sector_id; ?>]" 
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['nro_dietas']); ?>" 
                                           class="number-input" min="0" placeholder="0">
                                </td>
                                <td class="almuerzo-fields">
                                    <input type="number" name="nro_viandas_comunes[<?php echo $sector_id; ?>]" 
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['nro_viandas_comunes']); ?>" 
                                           class="number-input" min="0" placeholder="0">
                                </td>
                                ...
                                <td>
                                    <input type="time" name="hora_llegada[<?php echo $sector_id; ?>]" 
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['hora_llegada']); ?>" 
                                           class="time-input">
                                </td>
                                <td>
                                    <input type="time" name="hora_recibido[<?php echo $sector_id; ?>]" 
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['hora_recibido']); ?>" 
                                           class="time-input">
                                </td>
                                <td>
                                    <input type="text" name="firma_sector[<?php echo $sector_id; ?>]" 
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['firma_sector']); ?>" 
                                           class="firma-input" placeholder="Firma">
                                </td>
                                <td>
                                    <input type="text" name="aclaracion_sector[<?php echo $sector_id; ?>]" 
                                           value="<?php echo htmlspecialchars($datos_sectores[$sector_id]['aclaracion_sector']); ?>" 
                ...
                <div class="section-actions">
                    <button type="submit" class="btn" id="btn-guardar" onclick="return confirm('¿Está seguro de actualizar la distribución?')">
        ...
        <div class="global-actions">
            ...
            <a href="ver.php?id=<?= $id_distribucion ?>" class="btn btn-secondary">👁️ Ver Detalles</a>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function cambiarTipoComida(tipoId) {
                ...
                btnGuardar.innerHTML = '💾 Actualizar ' + nombreComida.split(' ')[1];
            ...
            cambiarTipoComida(<?php echo $datos['id_tipo_comida']; ?>);
